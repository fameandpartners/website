...
module Orders
  class LineItemPresenter < Orders::Shared::LineItemPresenter
    ...
    def customisations
      if personalizations?
        customs = Array.wrap(
            JSON.parse(item.customizations, object_class: OpenStruct).collect { |custom|
    ...
    def customisation_ids
      ...
      Array.wrap(JSON.parse(item.customizations, object_class: OpenStruct).collect{|x| x.customisation_value.id})
    end
