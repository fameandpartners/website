class Contact
  ...
  validates :linkedin, format: { with: /\A((http|https):\/\/)?(www.)?linkedin.com\//, message: 'use a valid LinkedIn URL' },
    if: Proc.new { |c| c.linkedin.present? && join_us_form? }

  validates :first_name, :last_name, :subject, :message, presence: true, if: :contact_form?
  validates :name, :interests, presence: true, if: :join_us_form?
