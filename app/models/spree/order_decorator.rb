Spree::Order.class_eval do
  ...
  def return_eligible_AC?
    self.return_type.blank? || self.return_type == 'C'|| (self.return_type == 'A' && (!self.promotions.select {|x| x.code.downcase.include? "deliverydisc"}.empty?)) #blank? handles older orders so we dont need to back fill
  end

  def return_eligible_B?
    self.return_type == 'B' && (!self.promotions.select {|x| x.code.downcase.include? "deliveryins"}.empty?)
  end
