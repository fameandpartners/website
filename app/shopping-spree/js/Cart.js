...
export default class Cart extends FirebaseComponent {
  ...
  addToCart(data, previousChildKey) {
    this.setState({
      totalInSharedCart:
        this.state.totalInSharedCart +
        Math.round(parseFloat(data.val().dress.price)),
    });
  ...
  checkoutOneItem(position, agent) {
    console.log(
      "Position = " + position + " length = " + this.state.myItems.length,
    );
  ...
  deleteItem(firebaseKey) {
    ...
    this.createFamebotMessage(
      "Oh No! " +
        this.props.name +
        " just removed an item from their cart.  You are now getting " +
  ...
  startListeningToFirebase() {
    ...
    this.cartDB = firebase.apps[0]
      .database()
      .ref(this.props.firebaseNodeId + "/cart");
      ...
      <div className="shopping-spree-cart">
        <div className="row header vertical-align">
          <div className="header__inner">
            <div
              className="back-to-spree col-md-4 col-xs-4"
              onClick={this.props.transitionToChat}
            >
        ...
        <div className="row">
          ...
          <div className="no-right-gutter col-xs-push-1 col-xs-6 text-right">
            <strong>
              ${(this.calculateCartTotal(this.state.myItems) -
                this.state.totalOff
              ).toFixed(2)}
            </strong>
          </div>
        ...
        <div className="shopping-spree-contents">
          <div className="row">
            <div className="col-xs-18">
              <ul className="cart-item-list">{this.state.myItems}</ul>
            </div>
          </div>
        </div>
      </div>
