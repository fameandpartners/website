...
class Products::DetailsResource
  ...
  private
    ...
    def related_outerwear
      ...
      related_outerwear = ActiveRecord::Base.connection.table_exists?(:spree_product_related_outerwear) ? product.related_outerwear : []
      related_outerwear.first(RELATED_OUTERWEAR_LIMIT).map do |recommended_product|
        Products::Related.new(product: recommended_product, site_version: site_version)
      end
    end
