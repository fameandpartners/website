...
export default class Cart extends FirebaseComponent {
  ...
  addToCart(data) {
    const { totalInSharedCart } = this.state;
    this.setState({
      totalInSharedCart: totalInSharedCart + Math.round(parseFloat(data.val().dress.price)),
    });
  ...
  deleteItem(firebaseKey) {
    ...
    //   }
    // }
    index = myItems.filter((item, i) => this.state.myItems[i].props.firebaseKey === firebaseKey);
  ...
  recalculateDiscount() {
    const discount = this.calculateDiscount(this.state.totalInSharedCart);
    ...
    return (
      <div className="shopping-spree-cart">
        ...
        <div className="row checkout-btn">
          <div
            className="no-right-gutter no-left-gutter col-xs-push-1 col-xs-10"
          >
