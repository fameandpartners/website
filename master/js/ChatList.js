    ...
    switch (data.val().type) {
      case 'text':
        this.setState(
          {
            messages:
                    this.state.messages.concat([<TextMessage
                      key={data.key}
                      text={data.val().value}
                      iconNumber={parseInt(data.val().from.icon, 10)}
      ...
      case 'share_dress':
        this.setState(
          {
            messages:
                    this.state.messages.concat([<DressMessage
                      key={data.key}
    ...
    return (
      <div className="chat-list">
        <div className="row chat-header">
          ...
          <div
            onClick={this.props.doneShoppingSpree}
            className="col-xs-1 col-xs-push-3"
            role="button"
          >
            Exit
          </div>
          <div
            ...
            className="col-xs-4 col-xs-push-3 text-right"
            role="button"
          >
