  ...
  describe 'associate user when guest checkout based on email, first name and last name' do
    context 'guest checkout' do
      ...
      it 'should associate user' do
        result = GuestCheckoutAssociation.associate_user_for_guest_checkout(spree_order: order, spree_current_user: nil, user_address:  {"bill_address_attributes" => {"email" => match_user.email, "firstname" => 'Something', "lastname" => 'Music'}})
        expect(result).to be true
        expect(order.user).to eq(match_user)
      end
      it 'should not associate user with correct email and incorrect first/last name' do
        result = GuestCheckoutAssociation.associate_user_for_guest_checkout(spree_order: order, spree_current_user: nil, user_address:  {"bill_address_attributes" => {"email" => match_user.email, "firstname" => "", "lastname" => ""}})
