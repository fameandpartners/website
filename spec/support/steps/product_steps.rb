module Acceptance
  module ProductSteps
    step 'I select :dress_size size' do |dress_size|
      ...
      sleep 0.2 # User interacting with sidebar + animation
      expect(page).to have_selector('.pdp-side-menu', visible: true)
    end

    step 'I select :skirt_length skirt length' do |skirt_length|
      sleep 0.2 # User interacting with sidebar + animation
      find('.height-selection input').set(skirt_length.downcase)
      sleep 0.2 # User interacting with sidebar + animation
      expect(page).to have_selector('.pdp-side-menu', visible: true)
    end

    step 'I select :height_value inch height' do |height_value|
      sleep 0.2 # User interacting with sidebar + animation
    ...
    step 'I save the profile' do
      # Trying to use a normal find/click pair caues crazy scrolling for some reason
      # Seems to be purely a capybara thing though - not an actual bug
      script = "$('.size-selection .btn-black').click()"
