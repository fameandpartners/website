...
class ProductColorValuesGrid
  ...
  column :variants, label: 'Active Variants' do |pcv|
    # binding.pry
    # pcv.product.variants.active.includes(:option_values).where('spree_option_values.id = ?', pcv.option_value.id).count
    #
    # Spree::OptionValue..joinswhere()
  end
end
