...
module Reports
  RSpec.describe SaleItems do
    ...
    subject(:report) { SaleItems.new(from: from_date, to: to_date) }

    describe '#new' do
      it 'requires from: & to:' do
        expect { SaleItems.new }.to raise_error ArgumentError
      end

      it do
        expect { SaleItems.new(from: from_date, to: to_date) }.not_to raise_error
      end
    end
    ...
    describe 'date coercion' do
      ...
      subject(:report) { SaleItems.new(from: date, to: date) }
