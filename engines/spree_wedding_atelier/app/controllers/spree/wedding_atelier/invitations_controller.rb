    ...
    class InvitationsController < Spree::WeddingAtelier::BaseController
      ...
      def accept
        ...
        if spree_user_signed_in? && @invitation.accept
          redirect_to wedding_atelier_event_path(@invitation.event_slug)
        elsif Spree::User.find_by_email(@invitation.user_email)
          @invitation.accept
          redirect_to wedding_atelier_sign_in_path({invitation_id: @invitation.id})
        else
          redirect_to wedding_atelier_signup_path({invitation_id: @invitation.id})
        end
      end
      ...
      def check_invitation_state(invitation)
        if invitation.state == 'accepted'
          ...
          redirect_to wedding_atelier_signup_path
        end
      end
    end
  end
