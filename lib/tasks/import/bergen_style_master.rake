namespace :bergen do
  ...
  desc 'Compare BERGEN STYLE MASTER CSV with existent Global SKUs, generating two CSV files: matched and not matched with Global SKUs'
  task compare_global_skus: :environment do
    file_path = ENV['FILE_PATH']

    raise 'FILE_PATH required' if file_path.blank?
    raise 'File must be a CSV' if File.extname(file_path) != '.csv'

    Importers::BergenStyleMaster.new(file_path).compare
  end
end
