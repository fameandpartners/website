...
namespace :newgistics do
  ...
  def generate_csv(shipments)
    csv_headers = %w[PurchaseOrder Name ShippedDate ArrivalDate ProductSKU ManifestDetailOriginalQty]
    ...
    Net::SFTP.start(configatron.newgistics.ftp_uri,
      ...
      sftp.upload!(temp_file, "\\FameandPartners\\input\manifests\\#{Date.today}.csv")
    end
  end
end
