    ...
    class Getprice < Base
      ...
      def export
        output = CSV.generate(col_sep: "\t") do |csv|
          ...
          @items.each do |item|
            csv << [item[:product].name, item[:variant_sku], item[:variant_sku], item[:product].id, @properties[item[:product].id].try(:[], :short_description), 'Dresses', 'Fame & Partners', item[:product].name, item[:image], "http://#{@config[:domain]}#{collection_product_path(item[:product])}", helpers.number_to_currency(item[:price]), 'Free delivery', item[:size], helpers.number_to_currency(item[:sale_price])]
          end
        end
