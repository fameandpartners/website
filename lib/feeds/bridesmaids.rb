...
module Feeds
  class Bridesmaids
            ...
            product.customization_visualizations.find_each do |cv|
              ...
              colors.each do |color|
                xml.item do
                  ...
                  xml.tag! "g:mpn", global_sku(product, customizations, color)
    ...
    def global_sku(product, customizations, color)
      style_number = product.master.sku
      product_name = product.name
      size = 'US0/AU4'
      height_value = 'standard'
      customization_ids = customizations.map {|c| c['customisation_value']['id']}

      gs = BridesmaidHelper.get_or_create_global_sku(style_number, product_name, size, color.downcase.parameterize, height_value, customizations)
      gs.sku
    end
