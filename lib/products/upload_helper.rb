...
module Products
  module UploadHelper
    ...
    def setup_collection(theme_json)
      theme = Theme.where(name: theme_json[:url].gsub('/', '')).first_or_create(name: theme_json[:url].gsub('/', ''), presentation: theme_json[:title], color:theme_json[:colors].to_json)
      theme_result = []
