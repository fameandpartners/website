  ...
  class MasterpassController < StoreController
    ...
    def cart
      ...
      items = current_order.line_items.map do |item|
        ::ShoppingCartItem.new(item.product.name + ' ' + item.variant.sku, item.quantity, item.price * 100, item.image.mini_url)
      end
      ...
      shopping_cart = ShoppingCart.new(current_order.currency, current_order.total * 100, items)
      shopping_cart_request = ShoppingCartRequest.new(@data.request_token, shopping_cart, provider.callback_domain)
      shopping_cart.shoppingCartItem.each do |i|
        i.description = i.description[0..99] if i.description.length > 100
        i.description = i.description[0..98] if i.description.last == "&"
      end
      @shopping_cart_response = ShoppingCartResponse.from_xml(provider.get_shopping_cart_response(shopping_cart_request))

      render :json => {
                 request_token: @data.request_token,
                 callback_domain: provider.callback_domain,
                 accepted_cards: provider.accepted_cards,
